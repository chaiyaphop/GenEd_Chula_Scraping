...
def get_course_codes(
    course_codes_file_path: str = 'course_codes_2025.csv'
) -> List[str]:
    df = pd.read_csv(course_codes_file_path, dtype=str)
    return df['course_code'].tolist()


def scrape_gened_courses(course_codes: List[str]) -> Dict[str, Dict[str, str]]:
    result = {}
            ...
            for lang in langs:
                ...
                try:
                    ...
                    if lang == "en":
                        ...
                        description_en = get_text_element("/html/body/div[2]/div/div/div[3]/p[2]", "Description")
                        faculty = get_text_element("/html/body/div[2]/div/div/div[2]/div[1]/p[2]", "Faculty")
                        category = "; ".join([cate.lstrip(", ").strip() for cate in get_text_elements("/html/body/div[2]/div/div/div[2]/div[2]//p[contains(@class, 'body1')]", "Category") if cate != ","])
                        ...
                        result[course_code].update({
                            "course_code": course_code if course_code else "N/A",
                            ...
                            "description_en": description_en if description_en else "N/A",
                        ...
                        description_th = get_text_element("/html/body/div[2]/div/div/div[3]/p[2]", "Description")

                        result[course_code].update({
                            "full_title_th": full_title_th if full_title_th else "N/A",
                            "description_th": description_th if description_th else "N/A",
...
if __name__ == '__main__':
    ...
    course_codes_file_path = 'course_codes_2025.csv'
    course_codes = get_course_codes(course_codes_file_path)
    scraped_data = scrape_gened_courses(course_codes)

    output_dir = 'scraped_data'
    output_filename = 'gened_courses_scraped.csv'
